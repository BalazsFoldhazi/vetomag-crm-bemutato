# Weboldal-szöveg (a bemutató oldalhoz)

> Ez a szöveg van beépítve az `index.html`-be. Itt szerkeszthető külön, ha módosítani akarsz
> rajta — utána át kell vezetni az oldalra is.

---

## Hero

**Címsor:** A partnertörzstől a kiküldött ajánlatig — *egy rendszerben*.

**Alcím:** Ügyfélkezelés, árajánlat, térkép, hívásnapló és piacfigyelés vetőmag- és
agrár-kereskedőknek. Nem általános CRM, amit mezőgazdaságra kell hajlítani.

**Gombok:** ▶ 60 másodperces bemutató · Prospektus (PDF)

## Videó-blokkok

- **Gyors bemutató** · 60 másodperc · hang nélkül is érthető
- **Teljes bemutató** · 3 perc · felirattal, hangszóró nélkül is követhető

## „Amit egy általános CRM nem tud" — hat kártya

1. **Agrár-adatok mezőként** — Hektár, AKG-részvétel és -terület, kultúrák, állatállomány,
   öntözés, tervezett és korábbi zöldítés, vásárlási motiváció, versenytárs-ajánlatok.
2. **Vetőmag-ajánlat másodpercek alatt** — Még a telefonhívás után, mobilról: koppintás a
   keverékre, és megy a PDF. Nem kell visszaérni az irodába. Keverék-kalkulátor, vetési norma,
   súly szerinti szállítási díj — mind magától.
3. **A terep is benne van** — Partnerek térképen, napi túra útvonalba rendezve, MePAR-blokkok,
   csapadéktérkép, műholdas vegetációs térkép.
4. **Telefon-integráció** — Bejövő hívásnál a partner előzménye a képernyőn. A telefonnapló
   magától frissül, a hívás a partnerhez kerül jegyzettel együtt.
5. **Piacfigyelés** — Versenytárs-termékek és árak egy helyen, ár-összehasonlítóval.
   Elvesztett ajánlatnál rögzül az ok és a versenytárs.
6. **AI-ügynökök, felügyelettel** — Adatdúsítás, partnerprofil, szakmai válaszok a saját
   dokumentumaidból, e-mail kampányok. Az asszisztens csak a saját CRM-adataidból dolgozik.

## Képernyőgaléria — felvezetés

**Nem makett — a működő rendszer.** Minden kép a futó alkalmazásból készült. A partnernevek,
telefonszámok és e-mail-címek kitaláltak: a felvételek anonimizált demó-adatbázisból származnak.

## Prospektus-blokk

**Nyolc oldal, nyomtatható.** Ugyanez leírva: a probléma, a modulok, a bevezetés menete és az
árazás alapjai. Jó, ha meg kell mutatni valakinek, aki nem néz videót.

## Záró blokk

Az oldalon **szándékosan nincs kapcsolatfelvételi felhívás**: ez most bemutató anyag, nem
értékesítési oldal. Az utolsó szekció a prospektus, utána közvetlenül a lábléc jön.

Ha később mégis értékesítési fázisba kerül, ez a szöveg illeszthető be zárásnak:

> **Nézd meg a saját adataitokkal.** Egy online bemutató fél óra. Megnézzük, mit vezettek ma és
> miben, és hogy ebből mi kerülne át a rendszerbe. Ha nem illik hozzátok, azt is megmondom.

## Lábléc / jogi

A MePAR-, támogatási és fajtaadatok tájékoztató jellegűek, hivatalos nyilvántartást nem helyettesítenek.
A képernyőképeken anonimizált demó-adatok szerepelnek.

---

## Ha később értékesítési fázisba kerül

- **Kapcsolatfelvétel.** Jelenleg nincs az oldalon. Ha kell, a prospektus-szekció után egy
  `<section class="final">` blokk a fenti szöveggel és egy gombbal (e-mail-cím vagy űrlap).
- **Árazás.** Szándékosan nincs ár sem az oldalon, sem a PDF-ben.
- **Megkeresések.** A kész hideg e-mailek és posztok a
  [`03-megkeresesek.md`](03-megkeresesek.md) fájlban várnak — azok is csak akkor kellenek,
  amikor tényleg elindul az értékesítés.
